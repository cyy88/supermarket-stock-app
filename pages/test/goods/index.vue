<template>
  <div class="app-container">
    ...
    <el-table ref="tables" v-loading="loading" :data="list" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="计价方式" align="center" width="80">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.priceType === 'weight'" type="warning" size="mini">称重</el-tag>
          <el-tag v-else type="success" size="mini">计件</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="价格" align="center" width="120">
        <template slot-scope="scope">
          <div v-if="scope.row.price">
            ...
            <span v-if="scope.row.priceType === 'weight'" style="font-size: 12px; color: #999;">元/kg</span>
            <span v-else style="font-size: 12px; color: #999;">元</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    resetQuery() {
      ...
      this.$refs.tables.sort('id', 'descending');
      this.handleQuery();
    },
    // 状态修改
    ...
    handleExport() {
      ...
      exportGoods(exportParams).then(response => {
        if (response.type && response.type.includes('application/json')) {
          ...
          reader.onload = e => {
            ...
            this.$modal.msgError(error.message || "导出失败");
          };
          reader.readAsText(response);
        } else {
          // 文件导出处理
          const blob = new Blob([response], { type: 'application/vnd.ms-excel' });
          const fileName = '商品列表_' + new Date().getTime() + '.xls';
          this.$modal.closeLoading();
