<template>
  <div class="sku-list">
    ...
    <div class="batch-setting">
      ...
      <el-input size="small" v-model="batch.price" :placeholder="priceType === 'weight' ? '单价(元/千克)' : '商品价格'" class="input"></el-input>
      <el-input size="small" v-model="batch.linePrice" :placeholder="priceType === 'weight' ? '划线单价(元/千克)' : '划线价格'" class="input"></el-input>
      <el-input size="small" v-model="batch.stock" placeholder="商品库存" class="input"></el-input>
      <el-input size="small" v-model="batch.weight" :placeholder="priceType === 'weight' ? '标准重量(千克)' : '商品重量(千克)'" class="input"></el-input>
    ...
    <el-table border :data="skuData.skuList">
      ...
      <el-table-column :label="priceType === 'weight' ? '单价(元/千克)' : '销售价格(元)'" align="center">
        <template slot-scope="scope">
          <el-input
            :readonly="disabled"
      ...
      <el-table-column :label="priceType === 'weight' ? '划线单价(元/千克)' : '划线价格(元)'" align="center">
        <template slot-scope="scope">
          <el-input
            :readonly="disabled"
...
export default {
  ...
  data() {
    return {
      ...
      // 批量设置
      batch: { price: '', skuNo: '', linePrice: '', weight: '', stock: 0 }
    };
  },
  watch: {
    "skuData.attrList": {
